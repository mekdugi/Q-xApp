  ...
  switch (controlMessage->m_e2SmRcControlHeaderFormat1->ric_Style_Type)
    {
      ...
      case RicControlMessage::ControlMessageServiceStyle::Energy_state: {
        uint16_t targetCellId = controlMessage->GetTargetCell ();
        uint16_t ctrlAction = controlMessage->m_e2SmRcControlHeaderFormat1->ric_ControlAction_ID;
        /* ctrl_act_id=1: sleep (TX off), ctrl_act_id=2: wake (TX on) */
        int txPower = (ctrlAction == 2) ? 30 : 0;
        for (uint32_t i = 0; i < mmWaveEnbNodes.GetN (); i++)
          {
            ...
            if (cell_id == targetCellId)
              {
                printf ("Cell Id %u %s ", cell_id, txPower > 0 ? "WAKE" : "SLEEP");
                Simulator::ScheduleWithContext (1, MilliSeconds (15), &SetBSTX, enbPhy, txPower, cell_id,
                                                txPower == 0);
              }
          }
        break;
