...
void
LteEnbNetDevice::EnqueuePendingHandover(uint64_t imsi, uint16_t targetCellId)
{
  std::lock_guard<std::mutex> lock (m_pendingHoMutex);
  m_pendingHoRequests.push_back ({imsi, targetCellId});
}

void
LteEnbNetDevice::ProcessPendingHandoverRequests()
{
  std::deque<PendingHandoverRequest> requests;
  {
    std::lock_guard<std::mutex> lock (m_pendingHoMutex);
    requests.swap (m_pendingHoRequests);
  }
  ...
  for (const auto &request : requests)
    {
      if (!m_rrc)
        {
          NS_LOG_UNCOND ("## LTE-RC HO: RRC is null, drop pending HO IMSI="
                         << request.imsi << " target=" << request.targetCellId);
          continue;
        }

      uint16_t rnti = m_rrc->GetRntiFromImsi (request.imsi);
      if (rnti == 0)
        {
          NS_LOG_UNCOND ("## LTE-RC HO: IMSI=" << request.imsi
                         << " rnti=0 in sim loop, skip pending HO");
          continue;
        }

      NS_LOG_UNCOND ("## LTE-RC HO: process pending IMSI=" << request.imsi
                     << " rnti=" << rnti << " target=" << request.targetCellId);
      m_rrc->TakeUeHoControl (request.imsi);
      m_rrc->PerformHandoverToTargetCell (request.imsi, request.targetCellId);
    }

  if (!m_stopSendingMessages)
    {
      Simulator::Schedule (MilliSeconds (10),
                           &LteEnbNetDevice::ProcessPendingHandoverRequests, this);
    }
}
  ...
  switch (styleType)
    {
        ...
        switch (controlMessage->m_e2SmRcControlHeaderFormat1->ric_ControlAction_ID)
          {
            case RicControlMessage::Connected_Mode_Mobility_Control_Action_ID::Handover_Control: {
              ...
              uint64_t imsi = 0;
              if (UEgnb && UEgnb->ran_UEID && UEgnb->ran_UEID->buf)
                memcpy(&imsi, UEgnb->ran_UEID->buf, UEgnb->ran_UEID->size);
              uint16_t targetCellId = controlMessage->GetTargetCell();

              if (imsi == 0) {
                printf("## LTE-RC HO: decoded IMSI=0, skip\n");
                fflush(stdout);
                break;
              }

              printf("## LTE-RC HO: queue IMSI=%lu target=%u\n",
                     (unsigned long)imsi, targetCellId);
              fflush(stdout);

              EnqueuePendingHandover(imsi, targetCellId);
              break;
            }
            default:
              break;
      ...
      default: {
        /* DRB control (style=1) and Energy (style=300) handled by mmWave cells, ignore here */
        break;
      }
    }
}
