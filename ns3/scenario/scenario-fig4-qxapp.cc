...
//   LogComponentEnable ("E2Termination", LOG_LEVEL_LOGIC);
    ...
    // LTE at center (anchor only)
    enbPositionAlloc->Add(centerPosition);

    // 3 mmWave equally spaced around center (NO co-location)
    double x, y;
    for (int8_t i = 0; i < (int8_t)nMmWaveEnbNodes; ++i) {
        x = isd_cell * cos((2.0 * M_PI * i) / nMmWaveEnbNodes);
        y = isd_cell * sin((2.0 * M_PI * i) / nMmWaveEnbNodes);
    ...
    // Fig. 4 deterministic UE positions
    // UE 1 & UE 4 near O-RU 1 / cell 2 (QoS-RA comparison pair, cell 2 RC works)
    // UE 2 near O-RU 2, UE 3 near O-RU 3
    ...
    uemobility.SetPositionAllocator(ueFixedPos);
