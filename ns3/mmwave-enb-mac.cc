...
MmWaveEnbMac::DoSchedConfigIndication(MmWaveMacSchedSapUser::SchedConfigIndParameters ind)
{
    ...
    for (unsigned iTti = 0; iTti < ind.m_slotAllocInfo.m_ttiAllocInfo.size(); iTti++)
    {
            ...
            ttiAllocInfo.m_tddMode == TtiAllocInfo::DL_slotAllocInfo)
        {
            ...
            {
                // Phase 3: if RNTI is handover-pending on ANY cell, skip gracefully
                {
                    bool hoPending = false;
                    for (auto &kv : MmWaveFlexTtiPfMacScheduler::s_handoverPendingRntis)
                    {
                        if (kv.second.count(rnti) > 0) { hoPending = true; break; }
                    }
                    if (hoPending)
                    {
                        NS_LOG_UNCOND("## MAC-HO: skip stale DCI for HO-pending rnti=" << rnti
                                      << " cell=" << m_cellId);
                        continue;
                    }
                }
                continue; // stale DCI on detached/uninitialized MAC — skip silently
            }
            else
            {
