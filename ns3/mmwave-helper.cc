...
MmWaveHelper::InstallSingleEnbDevice(Ptr<Node> n)
{
  ...
  */
    for (auto it = ccMap.begin(); it != ccMap.end(); ++it)
    {
        auto ccEnb = DynamicCast<MmWaveComponentCarrierEnb>(it->second);
        // Set MAC cellId (was missing — caused uninitialized m_cellId = 33199)
        Ptr<MmWaveEnbMac> ccMac = ccEnb->GetMac();
        if (ccMac)
        {
            ccMac->SetCellId(cellId);
        }
        Ptr<MmWaveEnbPhy> ccPhy = ccEnb->GetPhy();
        ccPhy->SetDevice(device);
        ccPhy->GetUlSpectrumPhy()->SetDevice(device);
        ccPhy->GetDlSpectrumPhy()->SetDevice(device);
        ccPhy->GetDlSpectrumPhy()->SetPhyRxDataEndOkCallback(
