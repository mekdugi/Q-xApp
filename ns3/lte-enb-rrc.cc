...
UeManager::ScheduleRrcConnectionReconfiguration ()
{
  ...
  switch (m_state)
    ...
    default:
      NS_LOG_UNCOND ("## HO-WARN: state=" << ToString (m_state) << " skipping"); return;
      break;
    }
}
    ...
    case HANDOVER_JOINING:
      {
        ...
        else
        {
          ...
          NS_LOG_UNCOND("## HO-COMPLETE: target cell " << m_rrc->m_cellId
                        << " IMSI=" << m_imsi << " rnti=" << m_rnti
                        << " sent SecondaryCellHandoverCompleted at " << Simulator::Now().GetSeconds());
          SwitchToState(CONNECTED_NORMALLY);
          NS_LOG_INFO("m_queuedHandoverRequestCellId " << m_queuedHandoverRequestCellId);
          if(m_queuedHandoverRequestCellId > 0)
          {
...
LteEnbRrc::GetUeManager (uint16_t rnti)
{
  NS_LOG_FUNCTION (this << (uint32_t) rnti);
  if (rnti == 0) {
    NS_LOG_UNCOND("## GetUeManager: rnti=0, returning nullptr");
    return nullptr;
  }
...
//  }
  ...
  if (it == m_ueMap.end ()) {
    NS_LOG_UNCOND("## GetUeManager: RNTI " << rnti << " not found in ueMap, returning nullptr");
    return nullptr;
  }
  return it->second;
}
  ...
  {  
    ...
    { 
      ...
      NS_LOG_UNCOND (Simulator::Now ().GetNanoSeconds ()/ 1.0e9
       ...
       try { SendHandoverRequest(rnti,targetCellId); } catch (...) { NS_LOG_UNCOND("## HO failed for rnti " << rnti); }    
      m_mmWaveCellSetupCompleted[imsi] = false;
    }
    else
    {
...
LteEnbRrc::PerformHandoverToTargetCell (uint64_t imsi, uint16_t targetCellId)
{
  ...
  if(!alreadyAssociatedImsi)
  {
    NS_LOG_UNCOND("## RC-HO: IMSI=" << imsi << " not associated yet, skip");
    return;
  }

  auto lastMmWaveCellIt = m_lastMmWaveCell.find(imsi);
  NS_LOG_UNCOND("## RC-HO: IMSI=" << imsi << " m_lastMmWaveCell=" <<
      (lastMmWaveCellIt != m_lastMmWaveCell.end() ? (int)lastMmWaveCellIt->second : -1) <<
      " target=" << targetCellId << " onHO=" << onHandoverImsi);

  if (lastMmWaveCellIt == m_lastMmWaveCell.end() || lastMmWaveCellIt->second == 0)
  {
    NS_LOG_UNCOND("## RC-HO: IMSI=" << imsi << " no valid previous mmWave cell, skip");
    return;
  }

  if (lastMmWaveCellIt->second == targetCellId)
  {
    NS_LOG_UNCOND("## RC-HO: IMSI=" << imsi << " already on target cell " << targetCellId << ", skip");
    return;
  }

  if(onHandoverImsi)
  {
    NS_LOG_UNCOND("## RC-HO: IMSI=" << imsi << " HO already in progress, skip to avoid state conflict");
    return;
  }

  // Phase 2: Cancel any existing auto-HO event for this IMSI
  HandoverEventMap::iterator handoverEvent = m_imsiHandoverEventsMap.find(imsi);
  if(handoverEvent != m_imsiHandoverEventsMap.end())
  {
    NS_LOG_UNCOND("## RC-HO: cancelling existing auto-HO for IMSI=" << imsi
                  << " (was targeting cell " << handoverEvent->second.targetCellId << ")");
    handoverEvent->second.scheduledHandoverEvent.Cancel();
    m_imsiHandoverEventsMap.erase(handoverEvent);
  }

  // Phase 2: Register RC HO in the same event map as auto-HO
  // Schedule via Simulator::Schedule so execution happens at a safe simulation boundary
  uint16_t rcHoDelayMs = 2;
  EventId scheduledHoEvent = Simulator::Schedule(MilliSeconds(rcHoDelayMs),
                                                  &LteEnbRrc::PerformHandover, this, imsi);
  HandoverEventInfo hoInfo;
  hoInfo.sourceCellId = lastMmWaveCellIt->second;
  hoInfo.targetCellId = targetCellId;
  hoInfo.scheduledHandoverEvent = scheduledHoEvent;
  m_imsiHandoverEventsMap.insert(std::pair<uint64_t, HandoverEventInfo>(imsi, hoInfo));

  NS_LOG_UNCOND("## RC-HO: IMSI=" << imsi << " scheduled HO " << lastMmWaveCellIt->second
                << " -> " << targetCellId << " in " << rcHoDelayMs
                << "ms via event map (same path as auto-HO)");
}
...
LteEnbRrc::DoRecvMcHandoverRequest(EpcX2SapUser::SecondaryHandoverParams params)
{
  ...
  NS_LOG_UNCOND("## MC-HO: DoRecvMcHandoverRequest IMSI=" << params.imsi
                << " rnti=" << rnti << " target=" << params.targetCellId
                << " on cell " << m_cellId
                << " at simTime=" << Simulator::Now().GetSeconds());
  if (rnti == 0) {
    NS_LOG_UNCOND("## MC-HO: rnti=0 for IMSI=" << params.imsi << " on cell " << m_cellId << ", skip");
    return;
  }
  Ptr<UeManager> ueMan = GetUeManager(rnti);
  if (!ueMan) {
    NS_LOG_UNCOND("## MC-HO: UeManager null for rnti=" << rnti << " on cell " << m_cellId << ", skip");
    return;
  }
  NS_LOG_UNCOND("## MC-HO: UeManager state=" << ueMan->GetState()
                << " for rnti=" << rnti << " on cell " << m_cellId);
  // Phase 3: Mark RNTI as handover-pending in scheduler, then delay SendHandoverRequest
  ns3::mmwave::MmWaveFlexTtiPfMacScheduler::MarkUeHandoverPending(m_cellId, rnti);

  // Delay SendHandoverRequest by a few TTIs so scheduler drains in-flight allocations
  uint16_t hoDelayMs = 5;
  NS_LOG_UNCOND("## MC-HO: rnti=" << rnti << " marked pending on cell " << m_cellId
                << ", scheduling SendHandoverRequest in " << hoDelayMs << "ms");
  Simulator::Schedule(MilliSeconds(hoDelayMs),
                      &LteEnbRrc::SendHandoverRequest, this, rnti, params.targetCellId);
}
...
LteEnbRrc::DoRecvUeContextRelease (EpcX2SapUser::UeContextReleaseParams params)
{
  ...
  uint16_t rnti = params.oldEnbUeX2apId;
    ...
    {
        NS_LOG_UNCOND("## HO-COMPLETE: source cell " << m_cellId
                    << " releasing UE to target " << params.sourceCellId
