...
EpcX2::DoSendHandoverRequest(EpcX2SapProvider::HandoverRequestParams params)
{
    ...
    if (m_x2InterfaceSockets.find(params.targetCellId) == m_x2InterfaceSockets.end()) {
      NS_LOG_UNCOND("## X2: No interface for targetCellId=" << params.targetCellId << ", skipping handover");
      return;
    }
  ...
  for (const auto& entry : m_x2InterfaceSockets)
{
    ...
    if (m_x2InterfaceSockets.find(params.oldCellId) == m_x2InterfaceSockets.end()) {
      NS_LOG_UNCOND("## X2: No interface for secondary HO oldCellId=" << params.oldCellId
                    << ", targetCellId=" << params.targetCellId << ", skipping handover");
      return;
    }
