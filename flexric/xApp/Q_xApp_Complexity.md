...
The released solvers have fixed dimensions (TS: $4\times3$, NES: $4\times2$, QoS-RA: $2\times4$). Consequently, the repository supports exact resource measurements at those dimensions but does not yet support an empirical size-scaling fit such as $aK\log K+b$. The asymptotic expressions below are obtained by generalizing the implemented reversible blocks; they are not fitted trendlines.
...
Equations (24)-(27) assume that $R$ is a power of two, as in the implemented $R=4$ circuit. For a non-power-of-two DRB set, an additional valid-label preparation or invalid-label oracle is required.
...
### B) CQF inherited reference curve

For a direct overlay with the existing CQF plot, its quantized reference curve may be retained after mapping CQF's user count to $K$ and using $N\simeq K/10$:
...
\left[
\frac{K^2-K}{s}
\log_2\log_2\left(\frac K{10}\right)
\right],
\qquad s=5.
}
\tag{32}
...
This is an inherited comparison curve, not a gate count measured from `dqna_ts.py`, `dqna_42.py`, or `dqna_qos.py`. Equation (32) should be used only in the same large-$K$, $N\simeq K/10$ regime as the CQF figure; Equation (31) is the more explicit QUBO gate model.

## 10.8 Classical baselines

For a common reference curve, the CQF cubic Hungarian fit can be retained:

$$
\boxed{
t_{\mathrm{class,CQF}}=0.182K^3\ \mathrm{ns}.
}
\tag{33}
$$

This coefficient belongs to the CQF reference machine and must be labeled as an inherited baseline rather than a measurement on the Q-xApp testbed.
...
where $S_{42}$ and $S_{\mathrm{QoS}}$ are the shot counts. The comparison curves are Equation (32) for the inherited CQF quantized baseline and Equation (33) for the inherited classical fit.

A numerical proposed-method scaling curve additionally requires fixed choices of $a$, $C$, $\epsilon$, MCX/rotation synthesis, and $\tau$. Since $a$ is instance-dependent and the released solvers are dimension-fixed, no numerical $K$-scaling trendline is reported here. A single line should also not average TS, NES, and QoS-RA runtimes, because they solve different assignment domains and use different amplification semantics.
...
2. TS has a quadratic query improvement in $1/a$ relative to repeated uniform sampling, while each query contains a nontrivial reversible oracle whose gate cost is given by Equations (3)-(11).
