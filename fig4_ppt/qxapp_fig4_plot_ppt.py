...
def main():
    ...
    for ext in ("png", "pdf", "svg", "eps"):
        figt.savefig(os.path.join(OUTDIR, f"fig4_throughput.{ext}"), dpi=300)
        figp.savefig(os.path.join(OUTDIR, f"fig4_power.{ext}"), dpi=300)
        figc.savefig(os.path.join(OUTDIR, f"fig4_combined.{ext}"), dpi=300)
    print(f"saved -> {OUTDIR}/fig4_throughput.*  fig4_power.*  fig4_combined.*")
