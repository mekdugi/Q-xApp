...
# Track file offsets to read only new lines without truncating
_file_offsets = {}


def read_new_lines(file_path):
    """Read only new lines appended since last read. Never truncates the file."""
    ...
    prev_count = _file_offsets.get(file_path, 1)  # skip header on first read
    # Reset offset if file was truncated (e.g. new simulation)
    if prev_count > len(lines):
        prev_count = 1
    new_lines = lines[prev_count:]
    _file_offsets[file_path] = len(lines)

    if not new_lines:
        return []

    data = [line.strip() for line in new_lines if line.strip()]
    return [headers] + data
